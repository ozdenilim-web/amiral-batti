...
// KUŞATMA — 3 kişilik, sabit üçgen hedefleme (A→B→C→A). V1: bot-only (2 bot + 1 gerçek oyuncu), online altyapısı sonraki aşama.
const SIEGE_PLAYERS = 3;
const SIEGE_SHOTS_PER_TURN = SHOTS_PER_TURN; // "standart oyundaki gibi" — aynı ateş kuralı
...
const TRANSLATIONS = {
  tr: {
    ...
    friendsTitle: "ARKADAŞLARIM", friendReqs: "GELEN İSTEKLER", friendsList: "ARKADAŞ LİSTESİ",
    friendAdd: "ARKADAŞ EKLE", friendPending: "İSTEK GÖNDERİLDİ", friendAccept: "KABUL", friendReject: "SİL",
  ...
  en: {
    ...
    friendsTitle: "MY FRIENDS", friendReqs: "INCOMING REQUESTS", friendsList: "FRIEND LIST",
    friendAdd: "ADD FRIEND", friendPending: "REQUEST SENT", friendAccept: "ACCEPT", friendReject: "DECLINE",
...
const Grid = memo(function Grid({ board, cellSize, onClick, onHover, onRightClick, onLongPress, onCellPointerDown, overlay, hoverCells, isDefense, shipColors, disabled, blinkCells, manualMarks, showShipStatus, onboardingHint, turnGlow, revealSweepMs, flatHit, flashCells }) {
  ...
  return (<div className="paint-box" style={{ background:`linear-gradient(135deg,${t.surfaceLight} 0%,${t.surface} 100%)`,border:"1px solid rgba(0,229,255,0.3)",borderRadius:10,padding:4,overflow:"hidden",boxShadow:"0 4px 20px rgba(0,0,0,0.4),inset 0 1px 0 rgba(255,255,255,0.06)" }}>
    ...
    {board.map((row,r) => { const rowDelayMs = revealSweepMs ? Math.round((r/(board.length||1))*revealSweepMs) : 0; return (<div key={r} style={{ display:"flex" }}><div style={{ width:cellSize,height:cellSize,display:"flex",alignItems:"center",justifyContent:"center",fontSize:cellSize>30?13:11,fontWeight:900,color:t.text,fontFamily:warrior,textShadow:"0 1px 2px rgba(0,0,0,0.6)",...labelGlowStyle }}>{r+1}</div>
      {row.map((val,c) => {
        const ovr=overlay?.[r]?.[c], isHov=hoverCells?.some(([hr,hc])=>hr===r&&hc===c), shipColor=shipColors?.[r]?.[c], isBlink=blinkCells?.some(([br,bc])=>br===r&&bc===c), isManual=manualMarks?.[r]?.[c], isRipple=rippleCell===`${r},${c}`, isFlash=flashCells?.some(([fr,fc])=>fr===r&&fc===c);
        ...
        if(isDefense){
          ...
          // KUŞATMA tarzı sade gösterim: isabet SADECE turuncu; batan gemi turuncu + ✕
          if(ovr==="hit"){bg="#ff8c2a";clr="#fff";shadow="inset 0 0 10px rgba(255,120,20,0.55), inset 0 1px 0 rgba(255,255,255,0.25)";}
          else if(ovr==="miss"){bg=t.miss;content="•";}
          else if(ovr==="sunk"){bg="#b8501a";clr="#fff";content="✕";shadow="inset 0 0 12px rgba(180,70,10,0.7)";}
...
function useCountUp(target, active, duration = 1300, from = 0) {
  const [value, setValue] = useState(from);
  useEffect(() => {
    if (!active) { setValue(from); return; }
    let raf, startTime;
    const tgt = Math.round(target || 0);
    const base = Math.round(from || 0);
    const step = (ts) => {
      if (!startTime) startTime = ts;
      const progress = Math.min(1, (ts - startTime) / duration);
      const eased = 1 - Math.pow(1 - progress, 3);
      setValue(Math.round(base + (tgt - base) * eased));
...
@keyframes shimmerPass{0%{transform:translate3d(0,0,0)}100%{transform:translate3d(600%,0,0)}}
        ...
        <div style={{ position:"relative",width:"clamp(140px, 42vw, 190px)",height:"clamp(140px, 42vw, 190px)",margin:"0 auto 4px",animation:"goShipIn 0.9s cubic-bezier(0.34,1.56,0.64,1) both" }}>
...
// === KUŞATMA SAVAŞ HARİTASI yardımcıları ===
function siegeMarkSunkOverlay(overlay, ships) {
  const g = (overlay || emptyGrid().map(r => r.map(() => null))).map(r => [...r]);
  const list = ships ? (Array.isArray(ships) ? ships : Object.values(ships)) : [];
  list.forEach(s => { const c = s.cells || []; if (c.length && c.every(([r, cc]) => g[r]?.[cc] === "hit" || g[r]?.[cc] === "sunk")) c.forEach(([r, cc]) => { if (g[r]) g[r][cc] = "sunk"; }); });
  return g;
}
function siegeShipColorGrid(ships) {
  const g = Array.from({ length: ROWS }, () => Array(COLS).fill(null));
  const list = ships ? (Array.isArray(ships) ? ships : Object.values(ships)) : [];
  list.forEach(s => { const sd = SHIPS.find(x => x.id === s.id); (s.cells || []).forEach(([r, c]) => { if (g[r]) g[r][c] = sd?.color || t.shipCell; }); });
  return g;
}
function siegeShipBoardGrid(ships) {
  const g = emptyGrid();
  const list = ships ? (Array.isArray(ships) ? ships : Object.values(ships)) : [];
  list.forEach(s => (s.cells || []).forEach(([r, c]) => { if (g[r]) g[r][c] = 1; }));
  return g;
}

// === KUŞATMA SAVAŞ HARİTASI — 3 oyuncu; her birinde SALDIRI + SAVUNMA. Tıklayınca büyür. ===
function SiegeReview({ players, showAll, concluded, cellSize, lang = "tr", onBack }) {
  const [focus, setFocus] = useState(null); // { i, mode }
  const visible = showAll ? [0, 1, 2] : [0];
  const thumb = 10;
  const bigC = Math.max(20, Math.min(30, cellSize || 26));
  return (
    <div style={{ minHeight:"100vh",minHeight:"100dvh",background:t.bg,color:t.text,fontFamily:mono,display:"flex",flexDirection:"column",alignItems:"center",padding:"18px 12px 30px",boxSizing:"border-box",overflowY:"auto" }}>
      <style>{ANIMS}</style>
      <div style={{ fontSize:18,fontWeight:900,letterSpacing:4,color:t.accent,fontFamily:warrior,textShadow:`0 0 15px ${t.accentGlow}`,marginBottom:14,textTransform:"uppercase" }}>{L(lang,"battleMap")}</div>
      <div style={{ width:"100%",maxWidth:420,display:"flex",flexDirection:"column",gap:12 }}>
        {visible.map(i => { const p = players[i]; if (!p) return null; return (
          <div key={i} style={{ background:"linear-gradient(145deg, rgba(12,21,41,0.95), rgba(8,14,30,0.98))",border:`2px solid ${p.me?t.accent:(p.alive?"rgba(255,255,255,0.12)":t.hit)}`,borderRadius:14,padding:"12px 12px 14px" }}>
            <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:10 }}>
              <span style={{ width:26,height:26,borderRadius:"50%",background:p.alive?`${t.accent}22`:"rgba(255,71,87,0.12)",border:`2px solid ${p.alive?t.accent:t.hit}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13 }}>{p.alive?p.avatar:"☠"}</span>
              <span style={{ fontSize:14,fontWeight:900,color:p.me?t.accent:t.text,fontFamily:warrior,letterSpacing:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{p.name}{p.me?(lang==="en"?" (YOU)":" (SEN)"):""}</span>
            </div>
            <div style={{ display:"flex",gap:12,justifyContent:"center" }}>
              {[{mode:"attack",label:L(lang,"attack"),clr:t.hit},{mode:"defense",label:L(lang,"defense"),clr:t.accent}].map(tab => (
                <button key={tab.mode} onClick={()=>setFocus({ i, mode:tab.mode })} style={{ background:"none",border:"none",padding:0,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:5 }}>
                  <div style={{ pointerEvents:"none",borderRadius:8,overflow:"hidden",border:`1px solid ${tab.clr}66` }}>
                    {tab.mode==="attack"
                      ? <Grid board={emptyGrid()} cellSize={thumb} overlay={p.atkOverlay} disabled />
                      : <Grid board={p.defBoard} cellSize={thumb} isDefense shipColors={p.defColors} overlay={p.defOverlay} disabled />}
                  </div>
                  <span style={{ fontSize:10,fontWeight:800,color:tab.clr,fontFamily:warrior,letterSpacing:2 }}>{tab.label} 🔍</span>
                </button>
              ))}
            </div>
          </div>
      ...
      {!showAll && <div style={{ marginTop:14,maxWidth:360,textAlign:"center",fontSize:11,color:t.gold,fontFamily:warrior,letterSpacing:1,lineHeight:1.5,background:"rgba(255,215,0,0.06)",border:"1px solid rgba(255,215,0,0.25)",borderRadius:10,padding:"10px 14px",animation:"pulse 2.2s ease-in-out infinite" }}>{lang==="en"?"Wait for the battle to end to see the other fleets' maps.":"Diğer filoların haritalarını görmek için savaşın bitmesini bekle."}{!concluded && <div style={{ fontSize:10,color:t.textDim,fontWeight:600,marginTop:5,letterSpacing:1 }}>⚔ {lang==="en"?"battle continues...":"savaş sürüyor..."}</div>}</div>}
      <button onClick={onBack} style={{ marginTop:20,padding:"12px 34px",background:"transparent",color:t.textDim,border:`1.5px solid ${t.border}`,borderRadius:10,fontSize:13,fontWeight:800,letterSpacing:2,cursor:"pointer",fontFamily:warrior }}>{L(lang,"backBtn")}</button>

      {focus && (() => { const p = players[focus.i]; if (!p) return null; return (
        <div onClick={()=>setFocus(null)} style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",backdropFilter:"blur(4px)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",zIndex:10000,padding:16 }}>
          <div onClick={e=>e.stopPropagation()} style={{ background:"linear-gradient(145deg, rgba(12,21,41,0.98), rgba(8,14,30,0.99))",border:`2px solid ${focus.mode==="attack"?t.hit:t.accent}`,borderRadius:16,padding:16,animation:"scaleUp 0.3s ease-out",maxWidth:"96vw" }}>
            <div style={{ fontSize:14,fontWeight:900,color:focus.mode==="attack"?t.hit:t.accent,fontFamily:warrior,letterSpacing:2,textAlign:"center",marginBottom:10 }}>{p.name} — {focus.mode==="attack"?L(lang,"attack"):L(lang,"defense")}</div>
            <div style={{ display:"flex",justifyContent:"center" }}>
              {focus.mode==="attack"
                ? <Grid board={emptyGrid()} cellSize={bigC} overlay={p.atkOverlay} disabled />
                : <Grid board={p.defBoard} cellSize={bigC} isDefense shipColors={p.defColors} overlay={p.defOverlay} disabled />}
            </div>
            <button onClick={()=>setFocus(null)} style={{ marginTop:12,width:"100%",padding:"10px 0",background:"rgba(255,255,255,0.05)",border:`1px solid ${t.border}`,borderRadius:10,color:t.textDim,fontFamily:warrior,fontWeight:800,letterSpacing:2,cursor:"pointer" }}>{L(lang,"close")}</button>
          </div>
        </div>
      ); })()}
    </div>
...
// ═══════════════════════════════════════════════════════════════════════
// ARKADAŞLIK SİSTEMİ
// Veri düzeni (Realtime Database):
//   friend_requests/{aliciUid}/{gonderenUid} = { fromName, fromAvatar, time }
//   friends/{uid}/{arkadasUid}              = { name, avatar, since }
// Kabul edildiğinde İKİ tarafın friends düğümüne de yazılır — böylece liste
// karşılıklıdır ve tek bir okumayla getirilir (karşı tarafın düğümünü ayrıca
// sorgulamak gerekmez). Çevrimiçilik bilgisi mevcut online_players düğümünden
// gelir; ayrı bir presence sistemi kurmaya gerek yok.
// ═══════════════════════════════════════════════════════════════════════
async function sendFriendRequest(fromUid, toUid, fromName, fromAvatar) {
  if (!fromUid || !toUid || fromUid === toUid) return false;
  try {
    // Karşı taraf zaten bize istek attıysa bekletmeden arkadaş ol.
    const inc = await get(ref(db, `friend_requests/${fromUid}/${toUid}`));
    if (inc.exists()) { await acceptFriendRequest(fromUid, toUid, inc.val(), fromName, fromAvatar); return true; }
    await set(ref(db, `friend_requests/${toUid}/${fromUid}`), {
      fromName: fromName || "Denizci", fromAvatar: fromAvatar || "⚓", time: Date.now(),
    });
    return true;
  } catch (e) { return false; }
}
async function acceptFriendRequest(myUid, fromUid, reqData, myName, myAvatar) {
  try {
    const now = Date.now();
    await Promise.all([
      set(ref(db, `friends/${myUid}/${fromUid}`), { name: reqData?.fromName || "Denizci", avatar: reqData?.fromAvatar || "⚓", since: now }),
      set(ref(db, `friends/${fromUid}/${myUid}`), { name: myName || "Denizci", avatar: myAvatar || "⚓", since: now }),
    ]);
    await remove(ref(db, `friend_requests/${myUid}/${fromUid}`)).catch(()=>{});
    await remove(ref(db, `friend_requests/${fromUid}/${myUid}`)).catch(()=>{});
    return true;
  } catch (e) { return false; }
}
async function rejectFriendRequest(myUid, fromUid) {
  try { await remove(ref(db, `friend_requests/${myUid}/${fromUid}`)); return true; } catch (e) { return false; }
}
async function removeFriend(myUid, friendUid) {
  try {
    await Promise.all([
      remove(ref(db, `friends/${myUid}/${friendUid}`)),
      remove(ref(db, `friends/${friendUid}/${myUid}`)).catch(()=>{}),
    ]);
    return true;
  } catch (e) { return false; }
}

// Arkadaş listesi ekranı — ayarlardan açılır. Gelen istekler, arkadaşlar (çevrimiçi
// göstergesiyle), doğrudan düello daveti ve arkadaş profili aynı ekranda.
function FriendsScreen({ myUid, myName, myAvatar, friends, requests, onlineMap, lang, onDuel, onClose }) {
  const [openUid, setOpenUid] = useState(null);   // profili açık arkadaş
  ...
  useEffect(() => {
    ...
    let alive = true;
    get(ref(db, `profiles/${openUid}`)).then(s => { if (alive) setProf(s.exists() ? s.val() : null); }).catch(()=>{});
    return () => { alive = false; };
  }, [openUid]);
  ...
  // === ARKADAŞ PROFİLİ ===
  if (openUid) {
    const f = friendList.find(x => x.uid === openUid) || {};
    const online = !!onlineMap[openUid];
    const total = prof?.totalGames || 0;
    const rate = total > 0 ? Math.round(((prof?.wins || 0) / total) * 100) : 0;
    const Stat = ({ label, value, color }) => (
      <div style={{ flex:1,textAlign:"center",padding:"10px 4px",background:"rgba(255,255,255,0.03)",border:`1px solid ${t.border}`,borderRadius:10 }}>
        <div style={{ fontSize:18,fontWeight:900,fontFamily:mono,color:color||t.text }}>{value}</div>
        <div style={{ fontSize:8.5,letterSpacing:1.4,color:t.textDim,fontFamily:warrior,marginTop:3 }}>{label}</div>
      </div>
    );
    return (<>
      <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:14 }}>
        <button onClick={()=>setOpenUid(null)} style={{ background:"none",border:"none",color:t.accent,fontSize:20,cursor:"pointer",padding:0,lineHeight:1 }}>‹</button>
        <span style={{ fontSize:16,fontWeight:900,color:t.text,fontFamily:warrior,letterSpacing:2 }}>{L(lang,"friendProfile")}</span>
      </div>
      <div style={{ display:"flex",flexDirection:"column",alignItems:"center",marginBottom:14 }}>
        <span style={{ width:64,height:64,borderRadius:"50%",background:`${t.accent}18`,border:`2px solid ${online?"#34d399":t.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:30,marginBottom:8 }}>{f.avatar || prof?.avatar || "⚓"}</span>
        <div style={{ fontSize:17,fontWeight:900,color:t.text,fontFamily:warrior,letterSpacing:1.5 }}>{prof?.displayName || f.name || "Denizci"}</div>
        <div style={{ fontSize:10,fontFamily:mono,marginTop:4,color:online?"#34d399":t.textDim,display:"flex",alignItems:"center",gap:5 }}>
          <span style={{ width:7,height:7,borderRadius:"50%",background:online?"#34d399":"#3d5163",display:"inline-block" }} />
          {online ? L(lang,"friendOnline") : L(lang,"friendOffline")}
        </div>
      </div>
      <div style={{ display:"flex",gap:8,marginBottom:9 }}>
        <Stat label={L(lang,"level")} value={prof?.level ?? 0} color={t.accent} />
        <Stat label="🎖" value={migrateHonor(prof)} color="#c084fc" />
        <Stat label={L(lang,"totalGames")} value={total} />
      </div>
      <div style={{ display:"flex",gap:8,marginBottom:14 }}>
        <Stat label={L(lang,"wins")} value={prof?.wins || 0} color="#34d399" />
  ...
  return (<>
    <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:14 }}>
      ...
      <span style={{ fontSize:16,fontWeight:900,color:t.text,fontFamily:warrior,letterSpacing:2 }}>{L(lang,"friendsTitle")}</span>
      <span style={{ marginLeft:"auto",fontSize:11,color:t.textDim,fontFamily:mono }}>{friendList.length}</span>
    </div>

    {reqList.length > 0 && (<>
      <div style={{ fontSize:9.5,fontWeight:800,letterSpacing:2,color:t.gold,fontFamily:warrior,marginBottom:7 }}>{L(lang,"friendReqs")} ({reqList.length})</div>
      {reqList.map(r => (
        <div key={r.uid} style={{ display:"flex",alignItems:"center",gap:10,padding:"10px 12px",marginBottom:8,borderRadius:11,background:"linear-gradient(180deg,rgba(60,45,20,0.5),rgba(30,22,10,0.5))",border:"1px solid rgba(201,161,94,0.4)" }}>
          <span style={{ width:34,height:34,borderRadius:"50%",background:"rgba(255,255,255,0.05)",border:`1px solid ${t.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:17,flexShrink:0 }}>{r.fromAvatar || "⚓"}</span>
          <span style={{ flex:1,minWidth:0,fontSize:13,fontWeight:800,color:"#f0d79a",fontFamily:warrior,letterSpacing:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{r.fromName || "Denizci"}</span>
          <button disabled={busy===r.uid} onClick={async()=>{ setBusy(r.uid); await acceptFriendRequest(myUid, r.uid, r, myName, myAvatar); setBusy(null); }}
            style={{ padding:"8px 12px",borderRadius:8,background:"linear-gradient(135deg,#34d399,#0d9488)",color:"#04231a",border:"none",fontSize:10.5,fontWeight:900,letterSpacing:1,cursor:"pointer",fontFamily:warrior }}>{L(lang,"friendAccept")}</button>
          <button disabled={busy===r.uid} onClick={async()=>{ setBusy(r.uid); await rejectFriendRequest(myUid, r.uid); setBusy(null); }}
            style={{ padding:"8px 10px",borderRadius:8,background:"transparent",color:t.hit,border:`1px solid ${t.hit}66`,fontSize:10.5,fontWeight:800,letterSpacing:1,cursor:"pointer",fontFamily:warrior }}>{L(lang,"friendReject")}</button>
        </div>
      ))}
      <div style={{ height:10 }} />
    </>)}
    ...
    {friendList.map(f => {
      ...
      return (
        <div key={f.uid} style={{ display:"flex",alignItems:"center",gap:10,padding:"10px 12px",marginBottom:8,borderRadius:11,background:"linear-gradient(180deg,rgba(22,36,50,0.6),rgba(12,22,32,0.6))",border:`1px solid ${online?"rgba(52,211,153,0.4)":"#26394b"}` }}>
          <button onClick={()=>setOpenUid(f.uid)} style={{ display:"flex",alignItems:"center",gap:10,flex:1,minWidth:0,background:"none",border:"none",padding:0,cursor:"pointer",textAlign:"left" }}>
            <span style={{ position:"relative",flexShrink:0 }}>
              <span style={{ width:36,height:36,borderRadius:"50%",background:"rgba(255,255,255,0.05)",border:`1px solid ${t.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18 }}>{f.avatar || "⚓"}</span>
              <span style={{ position:"absolute",right:-1,bottom:-1,width:11,height:11,borderRadius:"50%",background:online?"#34d399":"#3d5163",border:"2px solid #0d1a26" }} />
            </span>
            <span style={{ flex:1,minWidth:0 }}>
              <span style={{ display:"block",fontSize:13,fontWeight:800,color:"#dfe9f0",fontFamily:warrior,letterSpacing:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{f.name || "Denizci"}</span>
      ...
      <div style={{ width:"100%",maxWidth:400,display:"flex",flexDirection:"column",gap:6 }}>
        ...
        {players.map(p=>{const rank=getRankInfo(typeof p.honor==="number"?p.honor:((p.wins||0)*8+(p.losses||0)*3),lang);const alreadySent=sentInvite?.targetUid===p.uid;return(<div key={p.uid} style={{ display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:"linear-gradient(180deg,#20313f,#132030)",border:"1px solid #26394b",borderRadius:10,boxShadow:"inset 0 1px 0 rgba(255,255,255,0.05)" }}>
          ...
          {(() => {
            ...
            if (isFriend) return <span title="arkadaş" style={{ fontSize:13,color:"#34d399",flexShrink:0,lineHeight:1 }}>✓</span>;
            return (<button onClick={()=>onAddFriend && onAddFriend(p.uid)} title="arkadaş ekle"
              style={{ width:30,height:30,flexShrink:0,padding:0,background:reqIn?"linear-gradient(135deg,#34d399,#0d9488)":"rgba(52,211,153,0.10)",border:reqIn?"none":"1px solid rgba(52,211,153,0.45)",borderRadius:8,color:reqIn?"#04231a":"#34d399",fontSize:13,fontWeight:900,cursor:"pointer",fontFamily:warrior,display:"flex",alignItems:"center",justifyContent:"center",lineHeight:1 }}>+</button>);
          })()}
...
export default function Game() {
  ...
  // === ARKADAŞLIK ===
  const [friends, setFriends] = useState({});          // { uid: {name, avatar, since} }
  ...
  // Rakip botun uydurma seviye/şeref/altın bilgisi — sadece görsel, gerçek profil değil
  ...
  // Poker masası gibi: kazanan/kaybedenin altın yığını gözle görülür şekilde değişsin
  const salvoGoldStart = Math.max(0, salvoGoldBeforeAnteRef.current - SALVO_ANTE);
  const salvoGoldEnd = salvoGoldStart + (salvoResult ? (salvoResult.won ? SALVO_WIN_GOLD : salvoResult.draw ? SALVO_ANTE : 0) : 0);
  const salvoMyGoldAnim = useCountUp(salvoGoldEnd, salvoRevealActive, 900, salvoGoldStart);
  const botGoldStart = botStats.gold;
  const botGoldEnd = botGoldStart + (salvoResult ? (salvoResult.won ? -SALVO_ANTE : salvoResult.draw ? 0 : SALVO_ANTE) : 0);
  const salvoBotGoldAnim = useCountUp(botGoldEnd, salvoRevealActive, 900, botGoldStart);
  // === KUŞATMA modu (3 kişilik, sabit üçgen hedefleme) — V1 bot-only ===
  ...
  const siegeTurnIdxRef = useRef(0);
  const [siegeBotBoards, setSiegeBotBoards] = useState({});
  const siegeBotBoardsRef = useRef({});
  const [siegeBotShips, setSiegeBotShips] = useState({});
  const siegeBotShipsRef = useRef({});
  const [siegeNames, setSiegeNames] = useState({});
  const siegeMyShipCellsRef = useRef([]); // insanın yerleştirme onaylanınca dondurulan gemi hücreleri
  const [siegeReceived, setSiegeReceived] = useState({}); // {0:grid,1:grid,2:grid} — o oyuncunun tahtasına isabet eden atışlar
  const siegeReceivedRef = useRef({});
  const [siegeSelected, setSiegeSelected] = useState([]);
  const siegeSelectedRef = useRef([]);
  const [siegeSpectator, setSiegeSpectator] = useState(null); // { attacker, defender, cells, revealed }
  const [siegeGameOver, setSiegeGameOver] = useState(null); // { won } — İNSANIN sonucu (ekranı açar)
  ...
  const siegeClocksRef = useRef([CLOCK_SECONDS, CLOCK_SECONDS, CLOCK_SECONDS]);
  const [siegeElapsed, setSiegeElapsed] = useState(0); // toplam geçen süre (kronometre)
  // Kuşatma işaretleme (standart oyundaki gibi) — hedef tahtada şüpheli hücreler
  const [siegeMarks, setSiegeMarks] = useState(() => Array.from({ length: ROWS }, () => Array(COLS).fill(false)));
  const [siegeMarkMode, setSiegeMarkMode] = useState(false);
  const siegeMyHitsRef = useRef(0); // insanın toplam isabeti (sonuç ekranı için)
  const [siegeFlash, setSiegeFlash] = useState([]); // az önce ateş edilen hücreler — 1.5 sn yanıp söner
  const siegeFlashTimerRef = useRef(null);
  ...
  // ═══ ARKADAŞLIK DİNLEYİCİLERİ ═══
  // Arkadaş listesi, gelen istekler ve çevrimiçi kim var — üçü de canlı dinlenir,
  // böylece ekran açıkken durum anında güncellenir.
  useEffect(() => {
    if (!authUid) { setFriends({}); setFriendReqs({}); return; }
    const uF = onValue(ref(db, `friends/${authUid}`), s => setFriends(s.exists() ? s.val() : {}));
    const uR = onValue(ref(db, `friend_requests/${authUid}`), s => {
      ...
      if (typeof document !== "undefined" && document.visibilityState === "hidden") {
        Object.entries(val).forEach(([uid, d]) => {
          notifOnce("frq-" + uid + "-" + (d?.time || 0), () => {
            showNotif(L(appLangRef.current, "notifFriendReqTitle"),
                      L(appLangRef.current, "notifFriendReqBody")(d?.fromName || "Bir denizci"),
                      "ab-friend");
          });
        });
      }
  ...
  // Çevrimiçi haritası — mevcut presence düğümünden türetilir, ayrı sistem gerekmez.
  useEffect(() => {
    const unsub = onValue(ref(db, "online_players"), s => {
      const m = {};
      if (s.exists()) s.forEach(c => { m[c.key] = true; });
      setOnlineMap(m);
    });
    return () => unsub();
  }, []);
  ...
  // KUŞATMA sayacı — her saniye sıradaki saldıranın süresi bardan azalır; toplam kronometre artar; süre biterse elenme.
  useEffect(() => {
    if (phase !== "siege" || !siegeIntroDone) return;
    const id = setInterval(() => {
      if (siegeGameOverRef.current) return;
      setSiegeElapsed(e => e + 1);
      const cur = siegeTurnIdxRef.current;
      if (!siegeAliveRef.current[cur]) return;
      const nc = [...siegeClocksRef.current];
      nc[cur] = Math.max(0, nc[cur] - 1);
      siegeClocksRef.current = nc; setSiegeClocks(nc);
      if (nc[cur] <= 0) siegeHandleTimeout(cur);
    }, 1000);
    return () => clearInterval(id);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [phase, siegeIntroDone]);
  ...
  const renderTopBar = () => (
    <>
      ...
      {salvoMode && ["placing","salvo"].includes(phase) && (
            ...
            {settingsView === null && (<>
              ...
              <button onClick={()=>{ sfx.init(); sfx.play('click'); setSettingsView("friends"); }} style={rowBtnStyle}>
                ...
                <div style={{ flex:1,textAlign:"left" }}>
                  ...
                  <div style={rowSubStyle}>{L(appLang,"friendsSub")(Object.keys(friends).length)}</div>
                </div>
                {Object.keys(friendReqs).length > 0 && (
                  <span style={{ fontSize:10,fontWeight:900,color:"#04231a",background:"#34d399",borderRadius:9,padding:"2px 8px",fontFamily:mono,marginRight:4 }}>{Object.keys(friendReqs).length}</span>
                )}
                <span style={chevronStyle}>›</span>
              </button>
  ...
  const startSalvoBotGame = () => {
    ...
    salvoGoldBeforeAnteRef.current = safeGold(myProfile?.gold); // sonuç ekranındaki sayaç buradan başlayacak
    ...
    // Rakip botun uydurma seviye/şeref/altını — sadece görsel karşılaştırma için
    const myLvl = myProfile?.level || 1;
    setBotStats({
      level: Math.max(1, Math.min(60, myLvl + Math.floor(Math.random()*9) - 4)),
      honor: 30 + Math.floor(Math.random()*2400),
      gold: 200 + Math.floor(Math.random()*4800),
    });
    const bot = botPlaceShips();
    const name = BOT_NAMES[Math.floor(Math.random() * BOT_NAMES.length)];
    setIsBotGame(true); isBotGameRef.current = true;
  ...
  const startSiegeBotGame = () => {
    track("game_start", { mode: "siege3" });
    if (!playerName.trim()) { setMessage(L(appLang,"msgTypeName")); return; }
    const botA = botPlaceShips();
    const botB = botPlaceShips();
    const nameA = BOT_NAMES[Math.floor(Math.random() * BOT_NAMES.length)];
    let nameB = BOT_NAMES[Math.floor(Math.random() * BOT_NAMES.length)];
    ...
    siegeBotShipsRef.current = { 1: shipsA, 2: shipsB }; setSiegeBotShips(siegeBotShipsRef.current);
    setSiegeNames({ 0: playerName.trim(), 1: nameA, 2: nameB });
    ...
    siegeClocksRef.current = [CLOCK_SECONDS, CLOCK_SECONDS, CLOCK_SECONDS]; setSiegeClocks([CLOCK_SECONDS, CLOCK_SECONDS, CLOCK_SECONDS]);
    setSiegeElapsed(0);
    setSiegeMarks(Array.from({ length: ROWS }, () => Array(COLS).fill(false))); setSiegeMarkMode(false);
    siegeMyHitsRef.current = 0;
    if (siegeFlashTimerRef.current) clearTimeout(siegeFlashTimerRef.current); setSiegeFlash([]);
    setGameStartTime(Date.now());
    setPhase("placing");
    sfx.init(); sfx.playPlacementMusic();
  };

  // Bir atışı çözer: attacker → defender, cells listesi. İsabetleri defender'ın "received" tahtasına işler, elenme kontrolü yapar.
  const resolveSiegeShot = (attacker, defender, cells) => {
    const defShipCells = defender === 0
      ? siegeMyShipCellsRef.current
      : Object.values(siegeBotShipsRef.current[defender] || {}).flatMap(s => s.cells);
    const overlay = (siegeReceivedRef.current[defender] || emptyGrid().map(r => r.map(() => null))).map(row => [...row]);
    let hits = 0;
    cells.forEach(([r, c]) => { const hit = defShipCells.some(([sr, sc]) => sr === r && sc === c); overlay[r][c] = hit ? "hit" : "miss"; if (hit) hits++; });
    siegeReceivedRef.current = { ...siegeReceivedRef.current, [defender]: overlay };
    setSiegeReceived(siegeReceivedRef.current);
    const totalDefCells = defShipCells.length;
    const totalHitOnDef = overlay.flat().filter(v => v === "hit").length;
    const eliminated = totalDefCells > 0 && totalHitOnDef >= totalDefCells;
    if (eliminated) handleSiegeElimination(attacker, defender);
    return { hits, eliminated };
  };
  ...
  const handleSiegeElimination = (attacker, defender) => {
    ...
    nextTargets[attacker] = nextTargets[defender];
    siegeTargetOfRef.current = nextTargets; setSiegeTargetOf(nextTargets);
    const aliveCount = nextAlive.filter(Boolean).length;
    if (aliveCount <= 1) {
  ...
  const finishSiegeGame = (won, myKills = 0) => {
    ...
    const recvMe = siegeReceivedRef.current[0] || emptyGrid().map(r => r.map(() => null));
    const recvTgt = siegeReceivedRef.current[myTarget] || emptyGrid().map(r => r.map(() => null));
    const oppHitsCount = recvMe.flat().filter(v => v === "hit" || v === "sunk").length;
    const myHitsCount = siegeMyHitsRef.current;
    const targetShips = siegeBotShipsRef.current[myTarget] || null;
    const sunkCount = targetShips ? Object.values(targetShips).filter(s => (s.cells || []).length > 0 && s.cells.every(([r, c]) => recvTgt[r][c] === "hit" || recvTgt[r][c] === "sunk")).length : 0;
    setMyHits(myHitsCount); setOppHits(oppHitsCount); setIsWin(won);
    setWinner(won ? (appLang === "en" ? "Last fleet standing!" : "Ayakta kalan tek filo sensin!") : (appLang === "en" ? "Your fleet was wiped out." : "Filon tamamen batırıldı."));
    const myShipsObj = {}; placedShips.forEach((s, i) => { myShipsObj[i] = { id: s.id, cells: s.cells }; });
    setMyShipsData(myShipsObj); setOppShipsData(targetShips);
    setDefenseOverlay(recvMe); setAttackOverlay(recvTgt);
    ...
    if (authUid && myProfile) {
      ...
      bumpAch(a => {
        a.hits += myHitsCount; a.sunk += sunkCount;
        if (won) { a.botWins += 1; a.goldEarned += gold; a.winStreak += 1; a.bestWinStreak = Math.max(a.bestWinStreak, a.winStreak); a.lossStreak = 0; }
        else { a.winStreak = 0; a.lossStreak = (a.lossStreak || 0) + 1; }
      });
      bumpGlobalStats(1, sunkCount);
      bumpVoyageMatch();
  ...
  const runSiegeEngineStep = (lastAttacker) => {
    ...
    if (next === 0) return;
    // İnsan elenmişse (siegeGameOverRef set) botlar arka planda HIZLI bitirsin — insan haritayı beklemesin.
    const fast = !!siegeGameOverRef.current;
    setTimeout(() => runSiegeBotTurn(next), fast ? 40 : 1100 + Math.random() * 500);
  };

  // Bir botun turu: hedefine rastgele (henüz denenmemiş) hücrelerden ateş eder.
  // Hedef insan ise doğrudan uygulanır (insan kendi tahtasında görür). Hedef bot ise, insan üçüncü taraf olduğu için izleyici baloncuğu açılır.
  const runSiegeBotTurn = (attacker) => {
    if (!siegeModeRef.current || siegeStopRef.current || !siegeAliveRef.current[attacker]) return;
    const defender = siegeTargetOfRef.current[attacker];
    const receivedOverlay = siegeReceivedRef.current[defender] || emptyGrid().map(r => r.map(() => null));
    const already = new Set();
    receivedOverlay.forEach((row, r) => row.forEach((v, c) => { if (v) already.add(`${r},${c}`); }));
    const options = [];
    ...
    if (isSpectator && !siegeGameOverRef.current) {
      setSiegeSpectator({ attacker, defender, cells, revealed: 0 });
      sfx.init();
      cells.forEach((_, i) => { setTimeout(() => { if (siegeModeRef.current) { sfx.play('click'); setSiegeSpectator(prev => prev ? { ...prev, revealed: i + 1 } : prev); } }, 500 + i * 480); });
      setTimeout(() => {
        ...
        setTimeout(() => {
          if (!siegeModeRef.current) return;
          setSiegeSpectator(null);
          if (!siegeStopRef.current) runSiegeEngineStep(attacker);
        }, 5000);
      }, 700 + cells.length * 480);
    } else {
      const fast = !!siegeGameOverRef.current; // insan elenmiş: arka planda hızlı çöz
      setTimeout(() => {
        if (!siegeModeRef.current) return;
        if (!fast) { sfx.init(); sfx.play('click'); }
        resolveSiegeShot(attacker, defender, cells);
        // Sonuç 5 saniye ekranda kalsın (insan oynuyorsa), sonra sıra geçer.
        setTimeout(() => {
          if (!siegeModeRef.current) return;
          if (!siegeStopRef.current) runSiegeEngineStep(attacker);
        }, fast ? 40 : 5000);
  ...
  const siegeToggleCell = (r, c) => {
    if (siegeTurnIdxRef.current !== 0 || siegeGameOverRef.current) return;
    sfx.init(); sfx.play('click');
    setSiegeSelected(prev => {
      const exists = prev.some(([pr, pc]) => pr === r && pc === c);
      let next;
      if (exists) next = prev.filter(([pr, pc]) => !(pr === r && pc === c));
      else { if (prev.length >= SIEGE_SHOTS_PER_TURN) return prev; next = [...prev, [r, c]]; }
      siegeSelectedRef.current = next;
      return next;
    });
  };
  const siegeFire = () => {
    ...
    const res = resolveSiegeShot(0, defender, cells);
    siegeMyHitsRef.current += (res?.hits || 0);
    setSiegeSelected([]); siegeSelectedRef.current = [];
    // Rakibin platformunda 3 atış 1,5 saniye yanıp sönsün
    ...
    if (!siegeStopRef.current) setTimeout(() => runSiegeEngineStep(0), 5000);
  };

  // İşaretleme (standart oyundaki gibi): hedef tahtada şüpheli hücreyi işaretle/kaldır.
  const siegeToggleMark = (r, c) => {
    setSiegeMarks(prev => { const nm = prev.map(row => [...row]); nm[r][c] = !nm[r][c]; return nm; });
  };
...
`}</style>
      ...
      {(() => {
        ...
        return (
          <div style={{ width:"100%",maxWidth:400,marginTop:12,zIndex:1 }}>
            <style>{`
@keyframes dmShine{0%{transform:translate3d(0,0,0)}100%{transform:translate3d(500%,0,0)}}
...
@keyframes sailRing{0%{transform:scale(0.85);opacity:0.55}100%{transform:scale(1.5);opacity:0}}
  ...
  if (phase === "salvoreveal") {
    // TEK SABİT SAYFA: sonuç ekranı. Eskiden önce tam ekran ZAFER/BOZGUN yazısı (ResultBurst)
    // açılıp sonra bu sayfa geliyordu — aynı yazı iki kez görünüyordu. Artık doğrudan
    // sonuç ekranı açılır, başlık bu sayfanın tepesinde (daha küçük) yer alır.
    if (!salvoResult) {
  ...
  // === KUŞATMA — savaş ekranı: sıra bendeyse saldırı, bot bana ateş ediyorsa kendi tahtam, iki bot çarpışıyorsa izleyici baloncuğu ===
  if (phase === "siege") {
    const myTurn = siegeTurnIdx === 0;
    ...
    let boardMode = "own", overlayGrid = emptyGrid().map(row => row.map(() => null)), boardTitle = "";
    if (myTurn) {
      boardMode = "target";
      const received = siegeReceived[myTargetIdx] || overlayGrid;
      received.forEach((row,r) => row.forEach((v,c) => { if (v) overlayGrid[r][c] = v; }));
      siegeSelected.forEach(([r,c]) => { if (!overlayGrid[r][c]) overlayGrid[r][c] = "selected"; });
      boardTitle = `${nameFor(myTargetIdx).toUpperCase()} ${appLang==="en"?"WATERS":"SULARI"}`;
    } else if (isSpectating) {
      boardMode = "spectate";
      const base = siegeReceived[siegeSpectator.defender] || overlayGrid;
      base.forEach((row,r) => row.forEach((v,c) => { if (v) overlayGrid[r][c] = v; }));
      const defShipCells = siegeSpectator.defender === 0 ? siegeMyShipCellsRef.current : Object.values(siegeBotShips[siegeSpectator.defender] || {}).flatMap(s => s.cells);
      siegeSpectator.cells.slice(0, siegeSpectator.revealed).forEach(([r,c]) => { overlayGrid[r][c] = defShipCells.some(([sr,sc])=>sr===r&&sc===c) ? "hit" : "miss"; });
      boardTitle = `${nameFor(siegeSpectator.attacker).toUpperCase()} → ${nameFor(siegeSpectator.defender).toUpperCase()}`;
    } else {
      boardMode = "own";
      overlayGrid = siegeReceived[0] || overlayGrid;
      boardTitle = appLang==="en" ? "YOUR WATERS — UNDER FIRE" : "SENİN SULARIN — ATEŞ ALTINDA";
    }
    ...
    // Tek büyük tahta (11x11 + etiket satır/sütun = 12 birim) + ortada bekleme + altta 2 sıralı filo paneli.
    // Bölen 12 ve geniş dikey pay: taşma/kırpılma olmadan alttaki filo tamamen görünsün.
    const bigCell = Math.max(11, Math.min(32, Math.floor(Math.min((viewport.w - gutter - 8) / 12, (viewport.h - 384) / 12))));
    ...
    // Batan gemileri işaretle: tüm hücreleri vurulmuş gemiler "sunk" olur (üzerine ✕ gelir). Kopya üzerinde çalışır (state'i bozma).
    const markSunkCells = (grid, ships) => {
      const list = ships ? (Array.isArray(ships) ? ships : Object.values(ships)) : [];
      list.forEach(s => { const cells = s.cells || []; if (cells.length > 0 && cells.every(([r, c]) => grid[r]?.[c] === "hit" || grid[r]?.[c] === "sunk")) cells.forEach(([r, c]) => { if (grid[r]) grid[r][c] = "sunk"; }); });
      return grid;
    };
    ...
    // Saldırı narasyonu: "İlim → Hayreddin Paşa sularına saldırıyor"
    const atkName = nameFor(activeAttacker), defName = nameFor(activeDefender);
    ...
    return (<div style={{ ...appStyle, ...ambianceStyle(modeKey), height:"100dvh", maxHeight:"100dvh", overflow:"hidden", justifyContent:"flex-start", paddingTop:"calc(8px + env(safe-area-inset-top, 0px))", paddingBottom:8 }}><style>{ANIMS}</style>
      ...
      <div style={{ display:"flex",alignItems:"center",gap:9,marginBottom:4 }}>
        ...
        <span style={{ fontSize:11,fontWeight:900,color:t.textDim,fontFamily:mono,background:"rgba(255,255,255,0.05)",border:`1px solid ${t.border}`,borderRadius:7,padding:"2px 8px" }}>⏱ {formatTime(siegeElapsed)}</span>
      </div>
      {/* SAYAÇLAR — her oyuncuya 5 dk; sıradakinin barı azalır */}
      <div style={{ display:"flex",gap:6,width:"100%",maxWidth:420,marginBottom:5 }}>
        {[0,1,2].map(i => { const cl = siegeClocks[i] ?? CLOCK_SECONDS; const pct = Math.max(0, Math.min(100, cl/CLOCK_SECONDS*100)); const active = siegeTurnIdx===i && siegeAlive[i]; const dead = !siegeAlive[i]; const low = cl<=30; return (
          <div key={i} style={{ flex:1,padding:"3px 7px",borderRadius:7,background:active?"rgba(255,215,0,0.10)":"rgba(255,255,255,0.03)",border:`1px solid ${active?t.gold:t.border}`,opacity:dead?0.4:1 }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",gap:4 }}>
              <span style={{ fontSize:8,color:t.textDim,fontFamily:mono,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{dead?"☠ ":""}{nameFor(i)}</span>
              <span style={{ fontSize:10,fontWeight:900,color:low?t.hit:(active?t.gold:t.textDim),fontFamily:mono,flexShrink:0 }}>{formatTime(cl)}</span>
            </div>
            <div style={{ width:"100%",height:3,borderRadius:2,background:"rgba(0,0,0,0.45)",overflow:"hidden",marginTop:2 }}>
              <div style={{ width:`${pct}%`,height:"100%",borderRadius:2,background:low?t.hit:(active?`linear-gradient(90deg,${t.gold},#ffe066)`:t.accent),transition:"width 1s linear" }} />
            </div>
          </div>
        ); })}
      </div>
      {/* SALDIRI NARASYONU — büyük, deniz savaşı stili */}
      <div style={{ textAlign:"center",marginBottom:5,lineHeight:1.15,animation:"popIn 0.35s ease-out" }}>
        <span style={{ fontSize:16,fontWeight:900,fontFamily:warrior,letterSpacing:1,color:t.gold,textShadow:`0 1px 2px rgba(0,0,0,0.7), 0 0 14px ${t.goldGlow}` }}>{atkName}</span>
        <span style={{ fontSize:13,color:t.textDim,margin:"0 6px" }}>⚔</span>
        <span style={{ fontSize:16,fontWeight:900,fontFamily:warrior,letterSpacing:1,color:t.hit,textShadow:`0 1px 2px rgba(0,0,0,0.7), 0 0 14px ${t.hitGlow}` }}>{defName}</span>
        <div style={{ fontSize:10,fontWeight:700,color:t.textDim,fontFamily:warrior,letterSpacing:3,marginTop:1,textTransform:"uppercase" }}>{appLang==="en"?"is attacking":"sularına saldırıyor"}</div>
      </div>
      {isSpectating && <div style={{ fontSize:9,fontWeight:800,color:"#c084fc",marginBottom:2,fontFamily:warrior,letterSpacing:2,textAlign:"center",textTransform:"uppercase" }}>👁 {appLang==="en"?"WATCHING":"İZLİYORSUN"}</div>}

      {/* ÜST — SALDIRAN: tıklanan/ateş edilen büyük tahta; işaretleme ikonu sol üstte (sarı) */}
      <div key={`board-${activeAttacker}-${activeDefender}`} style={{ width:"100%",maxWidth:420,display:"flex",justifyContent:"center",animation:"siegeRiseUp 0.45s cubic-bezier(0.22,1,0.36,1) both" }}>
        <div style={{ position:"relative",display:"inline-block" }}>
          {boardMode === "target"
            ? <Grid board={emptyGrid()} cellSize={bigCell} overlay={displayGrid} disabled={false} onClick={siegeMarkMode?siegeToggleMark:siegeToggleCell} onRightClick={siegeToggleMark} onLongPress={siegeToggleMark} manualMarks={siegeMarks} flashCells={siegeFlash} />
            : boardFor(activeDefender, activeDefender===0 ? overlayGrid : displayGrid, false, bigCell)}
          {boardMode === "target" && (
            <button onClick={()=>setSiegeMarkMode(v=>!v)} title={siegeMarkMode?L(appLang,"markModeOn"):L(appLang,"markMode")}
              style={{ position:"absolute",top:6,left:6,zIndex:6,width:24,height:24,borderRadius:"50%",background:siegeMarkMode?t.gold:"rgba(10,14,23,0.95)",color:siegeMarkMode?t.bg:t.gold,border:`2px solid ${t.gold}`,fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",padding:0,lineHeight:1,boxShadow:siegeMarkMode?`0 0 10px ${t.goldGlow}`:"0 2px 8px rgba(0,0,0,0.5)" }}>⚑</button>
          )}
        </div>
      </div>
      <div style={{ fontSize:12,fontWeight:900,color:t.hit,fontFamily:mono,margin:"4px 0" }}>{appLang==="en"?"HITS":"İSABET"}: {hitCount}/20</div>
      {boardMode === "target" && (
      ...
      {/* OYUNDAN AYRIL onayı — diğer leave'lerle aynı */}
  ...
  if (phase === "siegeover") {
    ...
    if (showReview) {
      ...
      const siegePlayers = [0, 1, 2].map(i => {
        const defShips = shipsOf(i);
        const tgt = (i + 1) % 3; // sabit üçgen: i, (i+1)%3'e saldırır
        return {
          name: i === 0 ? (playerName || (appLang === "en" ? "YOU" : "SEN")) : (siegeNames[i] || "?"),
          avatar: i === 0 ? (myProfile?.avatar || "⚓") : "🤖",
          alive: !!siegeAlive[i], me: i === 0,
          defBoard: i === 0 ? defenseBoard : siegeShipBoardGrid(defShips),
          defColors: i === 0 ? shipColorMap : siegeShipColorGrid(defShips),
          defOverlay: siegeReceived[i], // ham (isDefense "sunk" render etmez; hit=patlama gemi üstünde)
          atkOverlay: siegeMarkSunkOverlay(siegeReceived[tgt], shipsOf(tgt)), // batan ✕ ile
        };
      });
      return <SiegeReview players={siegePlayers} showAll={siegeConcluded || isWin} concluded={siegeConcluded} cellSize={cellSize} lang={appLang} onBack={() => setShowReview(false)} />;
    }
    const chestProgressPct = Math.round((Object.keys(missionProgress).length / (dailyMissions.length || 3)) * 100);
    const myEloDiff = eloChange ? eloChange.myNew - eloChange.myOld : null;
    return (<><style>{ANIMS}</style>
      <GameOverScreen winner={winner} myHits={myHits} oppHits={oppHits} isWin={isWin} onNewGame={replaySiege} onHome={resetGame} onViewBoard={() => setShowReview(true)} goldEarned={myEloDiff ?? (goldAnim?.amount || 0)} myLevel={myProfile?.level || 0} chestProgressPct={chestProgressPct} lang={appLang}
        hookText={isWin ? L(appLang,"hookWin") : (revengeMult(safeAch(myProfile?.ach).lossStreak) > 1 ? L(appLang,"hookLossRevenge")(revengeMult(safeAch(myProfile?.ach).lossStreak)) : L(appLang,"hookLoss"))}
        onShowRewards={matchRewards ? (() => setRewardModalOpen(true)) : null}
        revengeStreak={!isWin ? safeAch(myProfile?.ach).lossStreak : 0} />
